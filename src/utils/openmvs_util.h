...
#include "utils/openmvs_data_interface.h"
              ...
              std::vector<std::string>& names, std::vector<int>& ptc)
{
    ...
    for(int i = 0; i < ncam; ++i)
    {
		...
		std::getline(in, str_line);		
		int idx1 = str_line.find_first_of('.');
		token = str_line.substr(0, idx1 + 4);

		std::string str_paras = str_line.substr(idx1 + 5, str_line.size() - idx1 - 5);
		std::stringstream sreader(str_paras);

		sreader >> f;
        for(int j = 0; j < rotation_parameter_num; ++j) sreader >> q[j];
		sreader >> c[0] >> c[1] >> c[2] >> d[0] >> d[1];
              ...
              std::vector<std::string>& names, std::vector<int>& ptc)
{
    ...
    for(int i = 0; i < ncam; ++i)
    {
		...
		//ss << std::setw(8) << std::setfill('0') << i << ".jpg";
...
bool LoadObjectSFMFile(const char* name, std::vector<CameraT>& camera_data, std::vector<Point3D>& point_data,
	std::vector<Point2D>& measurements, std::vector<int>& ptidx, std::vector<int>& camidx,
	std::vector<std::string>& names, std::vector<int>& ptc)
{
	std::ifstream in(name);
	...
	for (int i = 0; i < ncam; ++i)
	{
		...
		in >> f;
		double R[9], t[3];
		for (int j = 0; j < 9; ++j) in >> R[j];
		in >> t[0] >> t[1] >> t[2];

		camera_data[i].SetFocalLength(f);
		camera_data[i].SetMatrixRotation(R);
		camera_data[i].SetTranslation(t);
		camera_data[i].SetNormalizedMeasurementDistortion(0);
		names[i] = name_i;
	}
	...
	for (int i = 0; i < npoint; ++i)
	{
		...
		for (int j = 0; j < npj; ++j)
		{
			...
			camidx.push_back(cidx);    //camera index
			ptidx.push_back(i);        //point index						   
			measurements.push_back(Point2D(imx, imy));  //add a measurment to the vector
			nproj++;
		}
		point_data[i].SetPoint(pt);
		ptc.insert(ptc.end(), cc, cc + 3);
	...
	///////////////////////////////////////////////////////////////////////////////
	std::cout << ncam << " cameras; " << npoint << " 3D points; " << nproj << " projections\n";
	std::cout << ncam << " cameras; " << npoint << " 3D points; " << nproj << " projections" << std::endl;
	return true;
}
