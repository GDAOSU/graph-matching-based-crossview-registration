...
int linesMax, std::vector<line_float_t>& _lines)
{
	...
	for (; count > 0; count--)
	{
		...
		if (good_line)
		{
			for (size_t i = 0; i < pts_line.size(); i++)
			{
				int i1 = pts_line[i] / w;
				int j1 = pts_line[i] % w;
				...
				for (int p = -win_size; p < win_size; p++)
				{
					for (int q = -win_size; q < win_size; q++)
					{
						int i2 = i1 + p;
						int j2 = j1 + q;
						if (j2 < 0 || j2 >= w || i2 < 0 || i2 >= h) continue;

						adata = _accum;
						for (int m = 0; m < numangle; m++, adata += numrho)
						{
							int r = (int)round(j2 * ttab[m * 2] + i2 * ttab[m * 2 + 1]);
							r += (numrho - 1) / 2;
							adata[r]--;
						}
...
*/
int HoughLiner::HoughLineDetector(unsigned char * src, int w, int h, float scaleX, float scaleY, 
	float CannyLowThresh, float CannyHighThresh, float HoughRho, float HoughTheta, float MinThetaLinelength, 
	float MaxThetaGap, int HoughThresh, int _type, boundingbox_t bbox, std::vector<line_float_t>& lines)
{
	if (src == NULL)
		return 1;
...
int HoughLiner::HoughLineDetector(cv::Mat & edge_map, int _type, std::vector<line_float_t>& lines,
	double hough_rho, double hough_theta, double hough_thresh, double min_theta_linelength, double max_theta_gap)
{
	unsigned char *edge = edge_map.data;
	
	switch (_type)
	{
	case 0:
		hough_line_standard(edge, edge_map.cols, edge_map.rows,
			hough_rho, hough_theta, hough_thresh,
			min_theta_linelength, max_theta_gap,
			lines);

		break;

	case 1:
		hough_line_probabilistic(edge, edge_map.cols, edge_map.rows,
			hough_rho, hough_theta, hough_thresh,
			min_theta_linelength, max_theta_gap,
			lines);
