...
#ifndef OBJECTSFM_UTIL_WGS84_UTM_H_
#define OBJECTSFM_UTIL_WGS84_UTM_H_
...
namespace objectsfm
{
	...
	public:
		...
		static double R2D(double radian) {
			return radian / CV_PI * 180.0;
		}

		static cv::Vec3d lla_to_ecef(double lat, double lon, double alt)
		{
			...
			const double a2 = pow(WGS84_A, 2);
			const double b2 = pow(WGS84_B, 2);

			const double L = 1.0 / sqrt(a2 * pow(clat, 2) + b2 * pow(slat, 2));
		...
		**/
		static cv::Vec3d lla_to_utm(double lat, double lon, double alt, double a = WGS84_A, double b = WGS84_B)
		{
			...
			return cv::Vec3d(E * 1000, N * 1000, alt);
		}
		...
		**/
		...
		static cv::Vec3d ecef_to_lla(double x, double y, double z)
		{
			...
			return cv::Vec3d(R2D(lat), R2D(lon), alt);
		}
	};

}// namespace objectsfm

#endif // OBJECTSFM_UTIL_WGS84_UTM_H_
