...
namespace objectsfm
{
	...
	{
		ReprojectionErrorPoseCamXYZ(const double observed_x, const double observed_y, const double weight, const double f_init, const std::string mode)
			: observed_x(observed_x), observed_y(observed_y), weight(weight), f_init(f_init), mode(mode){}
		...
		// the client code.
		static ceres::CostFunction* Create(const double observed_x, const double observed_y, const double weight, const double f_init, const std::string mode)
		{
			if (mode == "fk1k2") {
				return (new ceres::AutoDiffCostFunction<ReprojectionErrorPoseCamXYZ, 2, 6, 3, 3>(
					new ReprojectionErrorPoseCamXYZ(observed_x, observed_y, weight, f_init, mode)));
			}
			else if (mode == "fk1k2dxdy") {
				...
				return (new ceres::AutoDiffCostFunction<ReprojectionErrorPoseCamXYZ, 2, 6, 7, 3>(
					new ReprojectionErrorPoseCamXYZ(observed_x, observed_y, weight, f_init, mode)));
			}
		}
