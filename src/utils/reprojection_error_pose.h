...
namespace objectsfm
{
	...
	{
		ReprojectionErrorPose(const double observed_x, const double observed_y, const double* point, const double* cam, const double weight, const double f_init, const std::string mode)
			: observed_x(observed_x), observed_y(observed_y), point(point), cam(cam), weight(weight), f_init(f_init), mode(mode){}
		...
		static ceres::CostFunction* Create(const double observed_x, const double observed_y, 
			const double* point, const double* cam, const double weight, const double f_init, const std::string mode)
		{
			return (new ceres::AutoDiffCostFunction<ReprojectionErrorPose, 2, 6>(
				new ReprojectionErrorPose(observed_x, observed_y, point, cam, weight, f_init, mode)));
		}
