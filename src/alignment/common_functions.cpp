...
void LineFunctions::subDivision(std::vector<std::vector<int> > &straightString, std::vector<cv::Point> &contour, int first_index, int last_index
	, double min_deviation, int min_size)
{
	int clusters_count = straightString.size();
	...
	else
	{
		...
		//if (1)
		{
			std::vector<int> straightStringCur;
			for (int i = first_index; i<last_index; ++i)
			{
				straightStringCur.push_back(i);
			}
			straightString.push_back(straightStringCur);
			//terminalIds.push_back(std::pair<int,int>(first_index, last_index));
		}
...
void PCAFunctions::Ori_PCA( PointCloud3d<double> &cloud, int k, std::vector<PCAInfo> &pcaInfos, double &scale, double &magnitd )
{
	double MINVALUE = 1e-7;
	int pointNum = cloud.pts.size();

	// 1. build kd-tree
	typedef KDTreeSingleIndexAdaptor< L2_Simple_Adaptor<double, PointCloud3d<double> >, PointCloud3d<double>, 3/*dim*/ > my_kd_tree_t;
...
void PCAFunctions::Ori_PCA2d(PointCloud2d<double>& cloud, int k, std::vector<PCAInfo2d> &pcaInfos, double &scale)
{
	double MINVALUE = 1e-7;
	int pointNum = cloud.pts.size();
...
#pragma omp parallel for
	for (int i = 0; i<pointNum; ++i)
	{
		...
		query_pt[0] = cloud.pts[i].x;  query_pt[1] = cloud.pts[i].y;
		double *dis_temp = new double[k];
		out_indices[i] = new size_t[k];
	...
	for (int i = 0; i < pointNum; ++i)
	{
		int idx = out_indices[i][9];
		double dx = cloud.pts[idx].x - cloud.pts[i].x;
		double dy = cloud.pts[idx].y - cloud.pts[i].y;
		double scaleTemp = sqrt(dx*dx + dy * dy);
		scale += scaleTemp;
	}
	scale /= pointNum;
...
#pragma omp parallel for
	for (int i = 0; i < pointNum; ++i)
	{
		...
		double h_mean_x = 0.0, h_mean_y = 0.0;
		for (int j = 0; j < ki; ++j)
		{
			int idx = out_indices[i][j];
			h_mean_x += cloud.pts[idx].x;
		...
		h_mean_x *= 1.0 / ki;  h_mean_y *= 1.0 / ki;

		double h_cov_1 = 0.0, h_cov_2 = 0.0, h_cov_3 = 0.0, h_cov_4 = 0.0;
		...
		for (int j = 0; j < k; ++j)
		{
			...
			h_cov_1 += dx * dx; h_cov_2 += dx * dy; h_cov_3 += dy * dx; h_cov_4 += dy * dy;
		}
		cv::Matx22d h_cov(
			h_cov_1, h_cov_2,
			h_cov_3, h_cov_4);
		...
		pcaInfos[i].idx_neigh.resize(ki);
		for (int j = 0; j<ki; ++j)
		{
			int idx = out_indices[i][j];
			pcaInfos[i].idx_neigh[j] = idx;
		}

		pcaInfos[i].normal = h_cov_evectors.row(1).t();

		double t = h_cov_evals.row(0).val[0] + h_cov_evals.row(1).val[0] + (rand() % 10 + 1) * MINVALUE;
		pcaInfos[i].lambda0 = h_cov_evals.row(1).val[0] / t;
...
void PCAFunctions::PCA2d(std::vector<cv::Point2d>& pts, int k, std::vector<double>& angles)
{
	//
	PointCloud2d<double> cloud;
	for (int i = 0; i < pts.size(); i++) {
		cloud.pts.push_back(PointCloud2d<double>::PtData(pts[i].x, pts[i].y));
	}
...
#pragma omp parallel for
	for (int i = 0; i < pointNum; ++i)
	{
		...
		cv::eigen(h_cov, h_cov_evals, h_cov_evectors);
		cv::Matx21d normal = h_cov_evectors.row(1).t();
		angles[i] = atan2(normal(1), normal(0)) + CV_PI;
...
void PCAFunctions::PCA2d(std::vector<cv::Point2d> &pts, int k, std::vector<cv::Point2d>& normals, std::vector<double> &angles)
{
	//
	PointCloud2d<double> cloud;
	for (int i = 0; i < pts.size(); i++) {
...
#pragma omp parallel for
	for (int i = 0; i < pointNum; ++i)
	{
		...
		cv::Matx21d normal = h_cov_evectors.row(1).t();
		//angles[i] = atan2(normal(1), normal(0)) + CV_PI;
		//normals[i] = cv::Point2d(normal(0), normal(1));

		normals[i].x = normal(0);
		normals[i].y = normal(1);
		angles[i] = atan2(normal(0), -normal(1)) + CV_PI;
...
void PCAFunctions::PCA2d(std::vector<Eigen::Vector2d>& pts, int k, std::vector<double>& angles, std::vector<double> &carvture)
{
	//
	PointCloud2d<double> cloud;
	for (int i = 0; i < pts.size(); i++) {
		cloud.pts.push_back(PointCloud2d<double>::PtData(pts[i](0), pts[i](1)));
	}
...
#pragma omp parallel for
	...
	// 3. PCA normal estimation
	angles.resize(pointNum);
	std::vector<double> curveture(pointNum, 0.0);
#pragma omp parallel for
	for (int i = 0; i < pointNum; ++i)
	{
		// 
		...
		cv::Matx21d normal = h_cov_evectors.row(1).t();
		angles[i] = atan2(normal(1), normal(0)) + CV_PI;
		double t = h_cov_evals(0) + h_cov_evals(1) + (rand() % 10 + 1) * MINVALUE;
		curveture[i] = 2 * h_cov_evals(1) / (h_cov_evals(0) + h_cov_evals(1));

		delete out_indices[i];
	}

	// sort carveture
	std::vector<Eigen::Vector2d> pts_new;
	std::vector<double> angles_new;
	std::vector<double> curveture_new;
	for (size_t i = 0; i < curveture.size(); i++)
	{
		if (curveture[i] < 0.3) {
			pts_new.push_back(pts[i]);
			angles_new.push_back(angles[i]);
			curveture_new.push_back(curveture[i]);
		}
	}
	pts = pts_new;
	angles = angles_new;
	carvture = curveture_new;

	delete[]out_indices;
	delete out_ks;
}

void PCAFunctions::Orthogonality(std::vector<Eigen::Vector2d>& pts, int k, std::vector<double>& scores)
{
	PointCloud2d<double> cloud;
	for (int i = 0; i < pts.size(); i++) {
		cloud.pts.push_back(PointCloud2d<double>::PtData(pts[i](0), pts[i](1)));
	...
	// 2. knn search
	std::vector<double> scales(pointNum);
	double **out_dis = new double*[pointNum];
	size_t **out_indices = new size_t *[pointNum];
#pragma omp parallel for
	for (int i = 0; i<pointNum; ++i)
	{
		...
		query_pt[0] = cloud.pts[i].x;  query_pt[1] = cloud.pts[i].y;		
		out_indices[i] = new size_t[k];
		out_dis[i] = new double[k];

		nanoflann::KNNResultSet<double> resultSet(k);
		resultSet.init(out_indices[i], out_dis[i]);
		index.findNeighbors(resultSet, &query_pt[0], nanoflann::SearchParams(10));
		scales[i] = out_dis[i][2];
	...
	scale_avg /= scales.size();

	double scale_sigma = 0.0;
	for (int i = 0; i < scales.size(); i++) {
		scale_sigma += pow(scales[i] - scale_avg, 2);
	}
	scale_sigma = sqrt(scale_sigma / scales.size());

	scale_avg = 0.0;
	int count_avg = 0;
	for (int i = 0; i < scales.size(); i++) {
		if ((scales[i] - scale_avg) < 2 * scale_sigma) {
			scale_avg += scales[i];
			count_avg++;
		}
	}
	...
	// 3. PCA normal estimation
	std::vector<double> angles(pointNum);
#pragma omp parallel for
	for (int i = 0; i < pointNum; ++i)
	{
		// 
	...
	// 4. Calculate orthogonality
	scores.resize(pointNum, 0.0);
	for (int i = 0; i < pointNum; i++) {
		double avg_dis = 0.0;
		for (int j = 0; j < k; ++j) {
			int idx = out_indices[i][j];
		...
		for (int m = 0; m < k; m++) {
			for (int n = 0; n < k; n++) {
				double dev = abs(angles[out_indices[i][m]] - angles[out_indices[i][n]]);
			}
		}
