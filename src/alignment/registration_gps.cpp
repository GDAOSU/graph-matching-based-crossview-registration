...
RegistrationGPS::RegistrationGPS()
{
}


RegistrationGPS::~RegistrationGPS(void)
{
}

void RegistrationGPS::Run()
{
	//scale_ = 0.62;
	gsd_ = 0.5;
...
void RegistrationGPS::PreSetPaths(std::string fold, std::string sate_dsm, std::string sate_ortho,
...
void RegistrationGPS::PreProssSatellite()
{
...
void RegistrationGPS::PreProssStreetview()
{
	AssignPt2Cams();
...
void RegistrationGPS::AssignPt2Cams()
{
...
void RegistrationGPS::VerticalBuildingDetection()
{
...
void RegistrationGPS::VerticalRefinement()
{
...
void RegistrationGPS::BuildingRefinement()
{
	// read in buildings
	std::string file_building = fold_ + "\\street_building.txt";
...
void RegistrationGPS::AlignmentXY()
{
	//////////////////////////////////
	...
	// read in gps
	std::ifstream iff = std::ifstream(file_street_gps_);
	int id_img;
	double lon, lat, x, y, z;
	std::map<int, cv::Point2d> imgname_gps;
	while (!iff.eof()) {
		iff >> id_img >> lat >> lon >> z;
		LLtoUTM(ellipsoid_id_, lat, lon, y, x, (char*)zone_id_.c_str());
		imgname_gps.insert(std::pair<int, cv::Point2d>(id_img, cv::Point2d((x - xs_) / dx_, (y - ys_) / dy_)));
	}
	...
	while (!iff.eof()) {
		...
		cams_name.push_back(std::stoi(name));
	}
	iff.close();	

	// accociate cam with gps
	std::vector<Eigen::Vector2d> cams_gps(cams.size());
	for (size_t i = 0; i < cams.size(); i++) {
		int imgname_i = cams_name[i];
		auto iter = imgname_gps.find(imgname_i);
		if (iter != imgname_gps.end()) {
			cams_gps[i] = Eigen::Vector2d(iter->second.x, iter->second.y);
		}
	}
	...
	//////////////////////////////////
	// step2: generate hypotheses
	//////////////////////////////////	
	std::string file_trans = fold_ + "\\trans_ori.txt";
	HypothesesViaGPS(cams, cams_gps, file_trans);

	//////////////////////////////////
	// step3: street building detection
	//////////////////////////////////
	std::vector<std::vector<Eigen::Vector2d>> street_buildings; 
	std::vector<cv::Vec2i> street_buildings_cam;
	StreetBuildingDetection(street_buildings, street_buildings_cam);
	if (1) {
		int rows = 3000;
		int cols = 3000;
		float xmin = -cols / 4;
	...
	cv::Mat mask_building = cv::imread(file_mask_building_, 0);
	std::vector<std::vector<cv::Point> > sate_contours;
	std::vector<cv::Vec4i> hierarchy;
	cv::findContours(mask_building, sate_contours, hierarchy, CV_RETR_LIST, CV_CHAIN_APPROX_NONE);
	std::vector<Eigen::Vector2d> sate_boundaries;
	for (int i = 0; i < sate_contours.size(); i++) {
		for (int j = 0; j < sate_contours[i].size(); j++) {
			sate_boundaries.push_back(Eigen::Vector2d(sate_contours[i][j].x, sate_contours[i][j].y));
		}
	}
	...
	//////////////////////////////////
	std::string file_transxy = fold_ + "\\xy_alignment.txt";
	std::vector<cv::Vec6d> trans_xy;
	if (!std::experimental::filesystem::exists(file_transxy)) {
		GlobalAlignment(street_buildings, sate_boundaries, trans_xy);

		// write out
		std::ofstream off(file_transxy);
		off << std::setprecision(12) << std::fixed;
		for (size_t i = 0; i < trans_xy.size(); i++) {
			off << street_buildings_cam[i].val[0] << " " << street_buildings_cam[i].val[1] << " ";
...
void RegistrationGPS::StreetBuildingDetection(std::vector<std::vector<Eigen::Vector2d>> &pts_buildings, std::vector<cv::Vec2i> &cams_buildings)
{
	// read in vetical
	std::string file_refined = fold_ + "\\vertical_refined.txt";
	std::ifstream ff1(file_refined);
	...
	pts_buildings = street_buildings;
	cams_buildings = street_cams_id;
}

void RegistrationGPS::TrajectorySegmentation(std::vector<Eigen::Vector2d> cams, std::vector<std::vector<int>>& cams_segs)
{
...
void RegistrationGPS::StreetBuildingSegmentation(std::vector<Eigen::Vector2d>& pts, float th_dev, std::vector<std::vector<Eigen::Vector2d>>& segments_pts, std::vector<std::vector<int>>& segments_ids)
{
	// step1: scaling
...
void RegistrationGPS::GlobalAlignment(std::vector<std::vector<Eigen::Vector2d>> src_list, 
	std::vector<Eigen::Vector2d> dst_list, std::vector<cv::Vec6d>& src_trans)
{
	std::vector<std::vector<Eigen::Vector2d>> src_list_ori = src_list;
	std::vector<Eigen::Vector2d> dst_list_ori = dst_list;
	...
	double id1, id2;
	double scale, angle, tx, ty;
	while (!iff.eof()) {
		iff >> id1 >> id2 >> scale >> angle >> tx >> ty;
		trans.push_back(cv::Vec6f(id1, id2, scale, angle, tx, ty));
	}
	iff.close();
	...
	CostTransformations(src_list, dst_list, trans, costs);
...
void RegistrationGPS::HypothesesViaGPS(std::vector<Eigen::Vector2d>& cams_street, std::vector<Eigen::Vector2d>& cams_gps, std::string file_trans)
{
	// 
	int n_correspondences = 0;
	std::vector<cv::Vec6f> matches;
	for (int i = 0; i < cams_street.size(); i++) {
		if ((cams_gps[i](0) + cams_gps[i](1)) != 0) {
			matches.push_back(cv::Vec6f(i, i, cams_street[i](0), cams_street[i](1), cams_gps[i](0), cams_gps[i](1)));
		}
	}

	int th_num = std::min(100, int(matches.size()));
	if (th_num < 2) {
		std::cout << "No enough gps, at least 2" << std::endl;
		return;
	}

	// calculate initial transform hypotheses
	std::vector<cv::Vec6f> trans_list;
	...
	while (i < matches.size()) {
		if (pts1.size() > 0 && pts1.size() % th_num == 0) {
			double s, angle;
			Eigen::Matrix2d R;
			Eigen::Vector2d t;
			double residual = 0.0;
			SimilarityTransformation(pts1, pts2, weight, R, t, s, residual);
			angle = atan2(-R(0, 1), R(0, 0));
			trans_list.push_back(cv::Vec6f(id_min, id_max, s, angle, t(0), t(1)));
			...
			pts1.push_back(Eigen::Vector2d(matches[i].val[2], matches[i].val[3]));
			pts2.push_back(Eigen::Vector2d(matches[i].val[4], matches[i].val[5]));
			weight.push_back(1.0);
			if (matches[i].val[0] < id_min) {
				id_min = matches[i].val[0];
			}
			if (matches[i].val[0] > id_max) {
				id_max = matches[i].val[0];
			}
		}
		i++;
	}

	// expand transform hypotheses
	std::vector<double> s_list, angle_list, tx_list, ty_list;
	for (size_t i = 0; i < trans_list.size(); i++) {
		s_list.push_back(trans_list[i].val[2]);
		angle_list.push_back(trans_list[i].val[3]);
		tx_list.push_back(trans_list[i].val[4]);
		ty_list.push_back(trans_list[i].val[5]);
	}

	cv::Vec3d s_info, angle_info, tx_info, ty_info;
	StaticVector(s_list, s_info.val[0], s_info.val[1], s_info.val[2]);
	StaticVector(angle_list, angle_info.val[0], angle_info.val[1], angle_info.val[2]);
	StaticVector(tx_list, tx_info.val[0], tx_info.val[1], tx_info.val[2]);
	StaticVector(ty_list, ty_info.val[0], ty_info.val[1], ty_info.val[2]);
	
	int angle_num = 10;
	float angle_min = angle_info.val[1] - 5.0 / 180.0*CV_PI;
	float angle_max = angle_info.val[2] + 5.0 / 180.0*CV_PI;
	float angle_step = (angle_max - angle_min) / angle_num;

	int tx_num = 40;
	float tx_min = tx_info.val[1] - 20.0;
	float tx_max = tx_info.val[2] + 20.0;
	float tx_step = (tx_max - tx_min) / tx_num;

	int ty_num = 40;
	float ty_min = ty_info.val[1] - 20.0;
	float ty_max = ty_info.val[2] + 20.0;
	float ty_step = (ty_max - ty_min) / ty_num;

	//
	scale_ = s_info.val[0];

	std::vector<cv::Vec6f> trans_all;
	for (size_t i = 0; i < angle_num; i++) {
		float angle_i = angle_min + i * angle_step;
		for (size_t m = 0; m < tx_num; m++) {
			float tx_m = tx_min + m * tx_step;
			for (size_t n = 0; n < ty_num; n++) {
				float ty_n = ty_min + n * ty_step;
				trans_all.push_back(cv::Vec6f(0, 0, s_info.val[0], angle_i, tx_m, ty_n));
			}
		}
	}
	...
	off << std::setprecision(12) << std::fixed;
	for (size_t i = 0; i < trans_all.size(); i++) {
		for (size_t j = 0; j < 6; j++) {
			off << trans_all[i].val[j] << " ";
		}
		off << std::endl;
	}
	off.close();
...
void RegistrationGPS::AlignmentZ()
{
	...
	std::string file_mask_road = fold_ + "\\mask_road.bmp";
...
void RegistrationGPS::DetectGround()
{
	std::string file_ground = fold_ + "\\ground.txt";
...
void RegistrationGPS::AlignmentXYZ()
{
...
void RegistrationGPS::PointOrthogonality(std::vector<Eigen::Vector2d>& pts, std::vector<double>& orthogonality)
{
...
// R = Rz*Ry*Rx
void RegistrationGPS::Rotation2Euler(Eigen::Matrix3d R, double & rx, double & ry, double & rz) {
...
void RegistrationGPS::Euler2Rotation(double rx, double ry, double rz, Eigen::Matrix3d & R)
{
...
void RegistrationGPS::StaticVector(std::vector<double>& data, double & v_avg, double & v_min, double & v_max)
{
	int num = data.size();
...
void RegistrationGPS::UniqueVector(std::vector<int> &data)
{
...
void RegistrationGPS::PCANormal(std::vector<Eigen::Vector2d>& pts, Eigen::Vector2d & normal, double & curve)
{
	int npts = pts.size();
...
void RegistrationGPS::ShowResult(std::vector<Eigen::Vector2d>& pts, double scale, cv::Mat & img)
{
	//
	std::vector<cv::Point2d> pts_all;
	for (size_t i = 0; i < pts.size(); i++) {
...
void RegistrationGPS::ShowResult(std::vector<Eigen::Vector2d>& pts, cv::Mat & img)
{
...
void RegistrationGPS::ShowResult(std::vector<Eigen::Vector2d>& src_ori, cv::Vec6f paras, std::vector<Eigen::Vector2d>& dst, cv::Mat & img)
{
...
void RegistrationGPS::AlignViaXYOffset(std::vector<Eigen::Vector2d>& src, std::vector<Eigen::Vector2d>& dst, Eigen::Vector2d & offset, double score)
{
	// get the bbox of dst
...
void RegistrationGPS::CostTransformations(std::vector<std::vector<Eigen::Vector2d>>& src_list, std::vector<Eigen::Vector2d>& dst_list,
	std::vector<cv::Vec6f>& trans, std::vector<std::vector<float>>& costs)
{
	// build kdtree 
	PointCloud2d<double> cloud;
	for (size_t i = 0; i < dst_list.size(); i++) {
		cloud.pts.push_back(PointCloud2d<double>::PtData(dst_list[i](0), dst_list[i](1)));
	}
	typedef KDTreeSingleIndexAdaptor< L2_Simple_Adaptor<double, PointCloud2d<double> >, PointCloud2d<double>, 2/*dim*/ > my_kd_tree_t;
	my_kd_tree_t index(2 /*dim*/, cloud, KDTreeSingleIndexAdaptorParams(10 /* max leaf */));
	index.buildIndex();
